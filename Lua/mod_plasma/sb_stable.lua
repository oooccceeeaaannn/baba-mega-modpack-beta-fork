        ...
        for _, feature in ipairs(featureindex[name]) do
            ...
            if rule[1] == name and rule[3] ~= "stable" and not is_name_text_this(rule[3]) then
                ...
                for _, tag in ipairs(tags) do
                    --[[ 
                        @Merge(metatext x plasma): Exclude subrules with "verbtext" tags from being a stable rule.
                        
                        Context: The metatext mod adds a bunch of new rule tags to mark subrules generated by forming rules involving meta# and text (Yes, "text" in the metatext mod is now a generator of subrules).
                        Subrules have the "verbtext" tag tend to be generated by "X mimic text/meta#. This change prevents stable from copying those subrules.


                        Below is a list of the newly introduced tags: 
                            visualmimic - on subrules made by "X mimic text/meta#"
                            dontadd - on subrules made by "meta# mimic meta#"
                            verbtext - on subrules made by "X !(is|has|become|make|write) text/meta#"
                            verbmeta# - extra information about the metalevel to put on subrules. The metalevel changes per subruletype
