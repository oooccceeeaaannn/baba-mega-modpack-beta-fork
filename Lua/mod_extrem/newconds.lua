	...
	for a,b in ipairs(params) do
		for c,d in ipairs(units) do
			...
			if unitid ~= d and d.strings[UNITNAME] == b then
				if floating(dunitid,unitid,x,y,d.values[XPOS],d.values[YPOS]) then --TODO: check tall, if floating() doesn't check
...
condlist["original"] = function(params,checkedconds,checkedconds_,cdata)
	...
	for a,b in ipairs(params) do
		if unit.originalname ~= b then --TODO: metatext stuff.
			pass = false
		end
	end
...
condlist["triggered"] = function(params,checkedconds,checkedconds_,cdata)
	...
	for a,b in ipairs(findallhere(x,y)) do
		if testcond({{"feeling", {"you"}}},b,x,y,{},cdata.limit) or testcond({{"feeling", {"you2"}}},b,x,y,{},cdata.limit) or testcond({{"feeling", {"3d"}}},b,x,y,{},cdata.limit) or testcond({{"feeling", {"puppet"}}},b,x,y,{},cdata.limit) or testcond({{"feeling", {"you+"}}},b,x,y,{},cdata.limit) then --TODO:replace with ws function.
			local bunit = mmf.newObject(b)

			if (x == bunit.values[XPOS]) and (y == bunit.values[YPOS]) then
				if floating(b,unitid,tilex,tiley) then
					return true,checkedconds,true
				end
			end
		end
...
condlist['refers'] = function(params,checkedconds,checkedconds_,cdata) --TODO: replace with mine version
